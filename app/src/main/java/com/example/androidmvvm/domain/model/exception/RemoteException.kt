package com.example.androidmvvm.domain.model.exception
...
class RemoteException(
    ...
    companion object {
        fun httpError(url: String, response: Response<*>?, retrofit: Retrofit): RemoteException {
            val message = response?.code().toString() + " " + response?.message()
            return RemoteException(message, url, response, Kind.HTTP, null, retrofit)
        }
        ...
        fun httpErrorWithObject(
            ...
            val message = response?.code().toString() + " " + response?.message()
            val error = RemoteException(message, url, response, Kind.HTTP_422_WITH_DATA, null, retrofit)
            error.deserializeServerError()
            return error
        }

        fun networkError(exception: IOException): RemoteException {
            return RemoteException(exception.message, null, null, Kind.NETWORK, exception, null)
        }
